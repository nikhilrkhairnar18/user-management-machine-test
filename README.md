...
An Angular and NgRx user-management application backed by `json-server`. The app includes mock login, a responsive user directory, and add, edit, and delete workflows.
...
Open `http://localhost:4200`. The demo accepts any non-empty username and password.

Run checks with:

```bash
npx nx test user-management --run
npx nx build user-management
```

The app keeps authentication and user CRUD state in NgRx. `UsersEffects` owns the HTTP calls to `json-server`, while standalone route components own the login and directory workflows.
...
- **3 Applications**
  ...
  - `user-management` - Angular and NgRx user directory with mock authentication
